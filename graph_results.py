...
c = 0
for line in reversed(list(open("brute_freqs.txt"))):
    if c == 1000:
        break
...
c = 0
mg_items = set()
for line in reversed(list(open("MG_90runs.txt"))):
    if line[0] == 'k':
        c = 0
        k = int(line.strip()[4:])
        recall = len(mg_items.intersection(true_items))
...
ax.set_xlabel('k size', fontsize=40, labelpad=510)
